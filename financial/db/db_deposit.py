...
def get_deposit_history_by_value(value, db:Session, status: Union[DepositHistoryStatus, None]= None):
    
    if status == None:
        return db.query(DbDepositHistory).filter(DbDepositHistory.value == value ).all()
    
    else:
        return db.query(DbDepositHistory).filter(and_(DbDepositHistory.value == value, DbDepositHistory.status == status) ).all()
