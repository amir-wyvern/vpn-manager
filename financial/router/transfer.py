...
def transfer_request(request: TransferRequest, admin_id: int=Depends(get_current_admin), db: Session=Depends(get_db)):

    request_id = uuid4().hex[:12]

    if request.user_id == request.to_user:
        logger.info(f'to_user is same from_user [request_id: {request_id} -user_id: {request.user_id}]')
    ...
    if config.transfer_lock == True:
        logger.info(f'transfer has locked [request_id: {request_id} -user_id: {request.user_id}]')
        raise HTTPException(status_code=403, detail={'internal_code':1020, 'message':'transfer has locked'})

    user_data = db_user.get_user(request.user_id, db)
...
def transfer_history(user_id: int, start_time: datetime, end_time: datetime, admin_id: int=Depends(get_current_admin), db: Session=Depends(get_db)):
